...
# ตั้งค่าหน้าเว็บแอปพลิเคชันแบบ Wide Layout คลีนๆ สไตล์ห้องเทรดโปร
st.set_page_config(
...
# ปรับแต่ง UI ด้วยดีไซน์หรูหรา ใช้งานง่าย และเน้นความชัดเจนของตัวอักษรในช่องค้นหาตามคำขอ
st.markdown("""
    <head>
    ...
    <style>
        ...
        input::placeholder {
            color: #9ca3af !important; /* สีของข้อความอธิบายแบบจางลง */
        }
...
# 4. เมนูข้าง (Sidebar) จัดกลุ่มฟังก์ชันเรียบง่าย สบายตา
with st.sidebar:
    st.markdown("""
    ...
    st.markdown("<label style='font-size: 13px; font-weight: 500; color: #9ca3af;'>🔍 พิมพ์ค้นหาชื่อหุ้นสหรัฐฯ</label>", unsafe_allow_html=True)
    ...
    # 📌 ปุ่มจัดการรายการโปรดแบบชัดเจน 2 ปุ่ม (เพิ่ม/ลบ) ตามคำขอ
...
if active_ticker:
    try:
        ...
        if df.empty:
            ...
            # 6. สร้างส่วนสรุปผลคำแนะนำระดับสายตา (Highlight Banner) ใหญ่ ชัดเจนที่สุด
            st.markdown(f"""
                <div style="
            ...
            # 7. การ์ดรายงานตัวเลขสำคัญ (Dashboard Cards Grid) ลอยเด่น ชัดเจน สไตล์โมเดิร์น
            st.markdown(f"""
                <div style="
                    display: grid;
            ...
            # 8. กล่องสรุปวิเคราะห์ข้อมูลย่อยจากสมองกล AI เข้าใจง่ายที่สุด
            ...
            # 9. โซนวาดกราฟเทคนิคัลระดับโปรที่ซูม-ลากดูราคาได้ (Plotly Interactive Chart)
            ...
            # 10. แผงตารางขยายดูรายงานราคาย้อนหลังรายแถวข้อมูลจริง
        ...
        st.error(f"❌ เกิดข้อผิดพลาดในการดึงข้อมูลหรือจัดระดับวิเคราะห์: {e}")        /* ตกแต่งกล่องกรอกข้อความ (Input fields) */
        div[data-baseweb="input"] {
            background-color: #1f2937 !important;
            border: 1px solid #374151 !important;
            border-radius: 10px !important;
